...
public class  ListButton<T> extends SelectBox<T> {
	...
	protected @Null Drawable getBackgroundDrawable () {
		if (isDisabled() && getStyle().backgroundDisabled != null) return getStyle()
				.backgroundDisabled;
		if (getScrollPane().hasParent() && getStyle().backgroundOpen != null) return getStyle()
				.backgroundOpen;
		if (isOver() && getStyle().backgroundOver != null) return getStyle().backgroundOver;
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		if (selected != null) {
			...
			font.draw(batch, name, x, y, 0, name.length(), width, 
					alignment, false, "...");
		}
	}
	...
	protected GlyphLayout drawItem(Batch batch, BitmapFont font, T item, float x, float y, float width) {
		...
		return font.draw(batch, string, x, y, 0, string.length(), width, 
				alignment, false, "...");
	}
